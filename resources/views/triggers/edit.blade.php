...
@section('title', 'Edit Trigger · WAPAPP for HubSpot')
...
@section('content')
    <div class="trigger-card">
        <div class="trigger-card-header">
            <div>
                <h1 class="trigger-card-title">Edit Trigger</h1>
                <p class="trigger-card-subtitle">
                    Update the HubSpot event mapping and WhatsApp template settings.
                </p>
            </div>
            <span class="badge-pill badge-wapapp">Editing: {{ $trigger->trigger_name }}</span>
        </div>
        ...
        <form method="POST" action="{{ route('triggers.update', $trigger->uuid) }}">
            @csrf
            @method('PUT')
            <div class="form-layout">
                ...
                <div class="section-card">
                    ...
                    <p class="section-note">
                        Adjust the HubSpot event and WhatsApp template for this trigger.
                    </p>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="hidden" name="event" id="event_input" value="{{ old('event', $trigger->event) }}"
                            required>
                                ...
                                <select class="form-select" name="template_uid" id="template_select" required>
                                    ...
                                    @foreach ($templates as $tpl)
                                        <option value="{{ $tpl['uid'] ?? '' }}" data-preview="{{ $tpl['body'] ?? '' }}"
                                            data-name="{{ $tpl['name'] ?? '' }}"
                                            {{ old('template_uid', $trigger->template_uid) == ($tpl['uid'] ?? '') ? 'selected' : '' }}>
                ...
                <div class="section-card section-card-right">
                    ...
                    <p class="section-note">
                        Update how HubSpot fields populate this WhatsApp message.
                    </p>
                    ...
                    <div class="mb-3">
                        ...
                        <div id="variables_container">
                            @foreach ($variables as $v)
                                @php $rowId = 'var_' . uniqid(); @endphp
                                <div class="variable-row">
                                    <div class="row g-2 align-items-center mb-2">
                                        <div class="col-4">
                                            <input type="text" name="vars[keys][]" class="form-control"
                                                placeholder="Variable name" value="{{ $v->var_key }}" />
                                        </div>
                                        ...
                                        <div class="col-1 text-end">
                                            <button type="button" class="btn btn-outline-danger btn-sm"
                                                onclick="this.closest('.variable-row').remove()">🗑️</button>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
